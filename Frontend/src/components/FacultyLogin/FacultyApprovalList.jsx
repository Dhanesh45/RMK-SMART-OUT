...
const FacultyApprovalList = () => {
    ...
    const handleApprove = (regno) => {
        setActionMap(prev => ({ ...prev, [regno]: 'approved' }));
    }

    const handleReject = (regno) => {
        setActionMap(prev => ({ ...prev, [regno]: 'rejected' }));
    }

    const handleApproveAll = () => {
        const updated = {};
        students.forEach(student => {
            updated[student.regno] = 'approved';
        });
        setActionMap(updated);
    }

    const handleRejectAll = () => {
        const updated = {};
        students.forEach(student => {
            updated[student.regno] = 'rejected';
        });
        setActionMap(updated);
    }

    const handleViewForm = (regno) => {
        navigate(`/view-form/${regno}`);
    }
        ...
        <div className="flex">
            ...
            <div style={{ width: "85%", height: "100vh", backgroundColor: "rgba(238, 238, 238, 0.5)" }}>
                            ...
                            <table style={{ borderCollapse: "collapse", width: "100%", borderRadius: "20px" }}>
                                <thead>
                                    <tr style={{ borderBottom: "8px solid rgba(217, 217, 217, 1)" }}>
                                        <th style={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 1, padding: '0.75%' }}>S.NO</th>
                                        <th style={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 1, padding: '0.75%' }}>NAME</th>
                                        <th style={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 1, padding: '0.75%' }}>REGISTER NUMBER</th>
                                        <th style={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 1, padding: '0.75%' }}>FORM DETAILS</th>
                                        <th style={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 1, padding: '0.75%' }}>VALIDATION</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {students.map(student => (
                                        <tr key={student.sno} style={{ backgroundColor: "white", margin: "10%", borderBottom: "8px solid rgba(217, 217, 217, 1)", borderRadius: "10px" }}>
                                            ...
                                            <td style={{ padding: "10px", textAlign: "center" }}>
                                                <button onClick={() => handleViewForm(student.regno)}>VIEW FORM</button>
                                            </td>
                                            <td style={{ padding: "10px", textAlign: "center" }}>
                                                <button
                                                    onClick={() => handleApprove(student.regno)}
                                                    disabled={actionMap[student.regno]}
                                                    style={{
                                                        fontWeight: "bold",
                                                        padding: "2%",
                                                        color: "white",
                                                        backgroundColor: actionMap[student.regno] === 'approved' ? 'rgba(14, 73, 71, 1)' : "green",
                                                        border: "none",
                                                        borderRadius: "5px",
                                                        margin: "0% 5% 0% 0%"
                                                    }}
                                                ...
                                                <button
                                                    onClick={() => handleReject(student.regno)}
                                                    disabled={actionMap[student.regno]}
                                                    style={{
                                                        fontWeight: "bold",
                                                        padding: "2%",
                                                        color: "white",
                                                        backgroundColor: actionMap[student.regno] === 'rejected' ? '#rgba(197, 9, 12, 1)' : "red",
                                                        border: "none",
                                                        borderRadius: "5px"
                                                    }}
                                                >
                ...
                <div style={{ width: "90%", display: "flex", justifyContent: "space-between", padding: "0.7% 5% 0% 5%" }}>
                    <button onClick={handleRejectAll} style={{ padding: "1% 3%", backgroundColor: "rgba(197, 9, 12, 1)", color: "white", fontWeight: "bold", fontSize: "14px", borderRadius: "25px", border: "none" }}>REJECT ALL</button>
